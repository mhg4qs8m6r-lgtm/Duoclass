...
export default function Atelier() {
  ...
  const atelierCategories = (categories || []).filter(c => {
    ...
    if (c.id === 'cat_creations') return true;
    return false;
  });
  ...
  // Albums fixes créations (sans "Projets en cours" qui n'est pas dans la liste demandée)
  const creationsFixedAlbums = (albums || []).filter(a => CREATIONS_FIXED_ALBUM_IDS.includes(a.id));

  // Albums de la catégorie sélectionnée
  const displayedAlbums = selectedCategory
    ? (() => {
        // Si "Images projets" sélectionné, montrer albums de cette catégorie
        if (selectedCategory === 'cat_mes_projets') {
          return (albums || []).filter(a => a.categoryId === 'cat_mes_projets');
        }
        // Si "CRÉATIONS" sélectionné, montrer les 7 albums fixes
        if (selectedCategory === 'cat_creations') {
          return creationsFixedAlbums;
        }
        // Sinon filtrer par categoryId
        return (albums || []).filter(a => a.categoryId === selectedCategory);
      })()
    : [];

  // Compter les albums par catégorie
  const getAlbumCount = (categoryId: string) => {
    if (categoryId === 'cat_creations') return creationsFixedAlbums.length;
  ...
  return (
    <MainLayout title={language === 'fr' ? 'Atelier' : 'Workshop'}>
      ...
      <div className="bg-emerald-500 text-white flex items-center justify-between py-2 px-4 text-sm font-medium -mx-6 -mt-6 mb-4">
        <span
      ...
      <div className="flex-1 overflow-auto">
        {showCreationForm ? (
          ...
          <div className="flex flex-row gap-0" style={{ width: '100%' }}>

            {/* Colonne 1 — Catégories */}
            <div className="p-2 min-w-0 flex-1">
              <h2 className="text-base font-medium mb-3 text-gray-800 flex items-center gap-2 border-b pb-2">
                {t('albums.categories')}
              </h2>
              <div className="space-y-2 flex flex-col items-center">
                {atelierCategories.length === 0 ? (
                  <p className="text-gray-400 text-center py-4 text-sm">
                    {language === 'fr' ? 'Aucune catégorie' : 'No category'}
                  </p>
                ) : (
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: selectedCategory === category.id ? (category.color || '#8B5CF6') : 'transparent'
                        }}
                      >
                        {selectedCategory === category.id && (
                      ...
                      {category.id !== 'cat_creations' && (
                        <button
                          ...
                          title={language === 'fr' ? 'Supprimer la catégorie' : 'Delete category'}
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
            ...
            {/* Colonne 2 — Albums */}
            <div className="p-2 min-w-0 flex-1 border-l">
              <h2 className="text-base font-medium mb-3 text-gray-800 flex items-center gap-2 border-b pb-2">
                {t('albums.title')}
              </h2>
              <div className="space-y-2 flex flex-col items-center">
                {!selectedCategory ? (
                  ...
                  <p className="text-gray-400 text-center py-4 text-sm">
                    {language === 'fr' ? 'Aucun album' : 'No album'}
                  </p>
                ) : (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-[520px] max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 space-y-5 text-sm text-gray-700">
              <div>
                ...
                <p>{"\u2192"} {language === 'fr' ? 'Cliquez sur "+ Créer un nouveau projet" en haut à droite' : 'Click "+ Create a new project" at the top right'}</p>
